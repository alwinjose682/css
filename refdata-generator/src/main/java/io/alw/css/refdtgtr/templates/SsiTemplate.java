...
public final class SsiTemplate extends TemplateBuilder<Ssi> {
    ...
    public SsiTemplate(Counterparty counterparty, Currency currency, TradeType product) {
        this(null, counterparty, currency, product);
    }

    private SsiTemplate(Ssi parent, Counterparty counterparty, Currency currency, TradeType product) {
        super(parent);
        this.utilities = SsiTemplateUtilities.singleton();
    ...
    @Override
    public SsiTemplate withDefaults() {
        ...
        ssiBuilder
                ...
                .product(product)
                .primary(isParentTemplate())
                .beneType(isParentTemplate() ? stringRefDataProvider.next(TestDataType.COUNTERPARTY_TYPE) : parent.beneType())
    ...
    protected SsiTemplate childTemplate(Ssi parent) {
        return new SsiTemplate(parent, this.counterparty, this.currency, this.product).withDefaults();
    }
}
