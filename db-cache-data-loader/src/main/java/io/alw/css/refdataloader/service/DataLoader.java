...
public class DataLoader {
    ...
    private void loadCacheFromDB_pagedData() {
        ...
        try {
            ...
            CountryAndCurrency countryAndCurrency = txro.execute("CountryAndCurrency", _ -> {
            ...
            cacheDataLoader.loadDataFromDB(getPagedDataSupplier("Nostro_Page", pageSize, dao::getNostroPaged));
            cacheDataLoader.loadDataFromDB(getPagedDataSupplier("Counterparty_Page", pageSize, dao::getCounterpartyPaged));
            cacheDataLoader.loadDataFromDB(getPagedDataSupplier("Ssi_Page", pageSize, dao::getSsiPaged));
            cacheDataLoader.loadDataFromDB(getPagedDataSupplier("CounterpartyNettingProfile_Page", pageSize, dao::getCounterpartyNettingProfilePaged));
            cacheDataLoader.loadDataFromDB(getPagedDataSupplier("CounterpartySlaMapping_Page", pageSize, dao::getCounterpartySlaMappingPaged));
    ...
    private <T extends ReferenceDataJpaEntity> Supplier<List<T>> getPagedDataSupplier(String ctxName, int pageSize, Function<PageRequest, List<T>> daoMethod) {
        PageRequest pageRequest = PageRequest.first(pageSize);
        return () -> {
            List<T> page = txro.execute(ctxName, _ -> daoMethod.apply(pageRequest));
            pageRequest.next();
            return page;
        };
    }
    ...
    private void loadCacheFromDb_lazyFetch_solution() {
        ...
        EntitiesAndNostros allRefData = txro.execute("getAllRefData_lazyFetch", _ -> {
    ...
    private void loadCacheFromDb_wholeData_MultipleBagFetchException() {
        ...
        List<CounterpartyEntity> cpEntities = txro.execute("getAllRefData_multipleBagFetchException", _ -> dao.getCounterpartyAndDependentData());
